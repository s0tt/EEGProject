...
for start_time in np.arange(config["t_start"], config["t_end"], config["t_sample"]):
    ...
    for subject in subjects:
        ...
        time = subject_data_model["time"]
        score = subject_data_model["score"]

        time_idx = [i for i, x in enumerate(time) if x >= start_time and x < start_time + config["t_sample"]]
        score = np.mean(score[time_idx[0]:time_idx[-1]+1])
        #peaks.append([peak["peak_time"], peak["peak_alt"]])
        score_total.append(score)
    
    score_mean += [np.mean(score_total)]
    ...
    statistics, pvalue = scipy.stats.ttest_1samp(score_total, 0.5, alternative='greater')
    pvalues.append(pvalue)
    # Calculate effect size with Cohan's d
    #cohensds.append((np.mean(score_total) - 0.5) / (np.sqrt((np.std(score_total) ** 2 + 0 ** 2) / 2)))
...
ax2.axhline(y=config["decode_alpha"]*100, linestyle='--', label="alpha={} %".format(config["decode_alpha"] * 100))  # Horizontal line indicating chance (50%)
...
ax3.pcolormesh(time_lst, [0,1], [pvalues < config["decode_alpha"]], cmap="RdYlGn")
